...
export interface RepoInternalBaseline {
  r_squared: number;
  beta_workload: number;
  beta_churn: number;
  beta_first_time: number;
  beta_files: number;
  intercept: number;
}

export interface AnnotatedPRItem {
  pr_id: string;
  number: number;
  title: string;
  author_id: string;
  ...
  workload: number;
  latency_hrs: number;
  observed_latency_hrs: number;
  expected_latency_hrs: number;
  residual_hrs: number;
  pr_z_score: number;
  audit_verdict: string;
  verdict_badge: 'normal' | 'delayed' | 'fast_tracked';
...
export interface SingleRepoAuditResponse {
  repo_name: string;
  total_prs_analyzed: number;
  repo_internal_baseline: RepoInternalBaseline;
  observed_mean_latency_hrs: number;
  observed_median_hrs: number;
  expected_latency_hrs: number;
  residual_hours: number;
  residual_z_score: number;
  is_anomaly: boolean;
  confidence_interval_95: [number, number];
  internal_variance_std: number;
  std_residual_repo?: number;
  anomalous_prs_count: number;
  anomalous_prs_pct: number;
  diagnosis: string;
  status: string;
  status_code: AnomalyStatusCode;
  badge_type: 'anomaly' | 'expedited' | 'normal';
