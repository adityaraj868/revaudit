...
import { RepositoryAuditRecord } from '../types/audit';
import { AlertCircle, Filter, HelpCircle } from 'lucide-react';

interface AnomalyPlotProps {
  repositories: RepositoryAuditRecord[];
}

export const AnomalyPlot: React.FC<AnomalyPlotProps> = ({ repositories }) => {
  ...
  const sortedData = [...repositories].sort((a, b) => {
    if (sortBy === 'latency') return b.observed_latency_hours - a.observed_latency_hours;
  ...
  const chartData = sortedData.map(r => ({
    ...
    category: r.category,
    observed: r.observed_latency_hours,
    expected: r.expected_latency_hours,
    ciLower: r.confidence_interval_95[0],
    ciUpper: r.confidence_interval_95[1],
    errorRange: [
      Math.max(0, r.expected_latency_hours - r.confidence_interval_95[0]),
      Math.max(0, r.confidence_interval_95[1] - r.expected_latency_hours)
    ],
    zScore: r.z_score,
    status: r.status,
    ...
    shiftInjected: r.synthetic_shift_injected
  }));
        ...
        <div className="bg-slate-900/95 border border-slate-700 p-4 rounded-xl shadow-2xl text-xs space-y-2 max-w-xs backdrop-blur-md">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            ...
            <span className={`px-2 py-0.5 rounded-full font-mono text-[10px] border ${
              data.badgeType === 'anomaly'
                ? 'bg-rose-500/10 text-rose-300 border-rose-500/30'
                : data.badgeType === 'expedited'
  ...
  return (
    <div className="p-6 rounded-2xl bg-slate-900 border border-slate-800 shadow-sm space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            <h3 className="text-lg font-bold text-white">Observed vs. Expected Review Latency</h3>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Comparing empirical review turnaround against multi-level model baseline across 15 repositories after adjusting for patch size, reviewer workload, and author familiarity.
          </p>
        </div>
      ...
      {/* Bar Chart */}
      <div className="h-88 w-full pt-4" style={{ height: '360px' }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Bar
              dataKey="observed"
              name="Observed Turnaround"
              radius={[4, 4, 0, 0]}
            >
              {chartData.map((entry, index) => {
                let color = '#38bdf8'; // normal in control
                if (entry.badgeType === 'anomaly') color = '#f43f5e'; // delayed bottleneck
                if (entry.badgeType === 'expedited') color = '#10b981'; // expedited flow
      ...
      <div className="pt-2 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-400 border-t border-slate-800">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-1.5">
            ...
            <span>Within Process Bounds (|Z| ≤ 1.96)</span>
          </div>
          <div className="flex items-center space-x-1.5">
            <span className="h-3 w-3 rounded-sm bg-[#f43f5e]"></span>
            <span>Bottleneck Anomaly (|Z| &gt; 2.0)</span>
          </div>
        </div>

        <div className="text-[11px] text-slate-500 font-mono">
          Threshold: |Z| &gt; 2.0 (p &lt; 0.05) triggers process capacity audit
        </div>
      </div>
    </div>
  );
